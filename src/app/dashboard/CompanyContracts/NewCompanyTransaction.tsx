...
interface NewCompanyTransactionProps {
  isOpen: boolean;
  onClose: () => void;
  companyId: number;
}

export default function NewCompanyTransaction({
  isOpen,
  onClose,
  companyId,
}: NewCompanyTransactionProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await axios.post("/api/company-transactions", data);
      return response.data;
    },
    onSuccess: () => {
      toast.success("تم إنشاء معاملة الشركة بنجاح!");
      queryClient.invalidateQueries({ queryKey: ["companyTransactions", companyId] });
      form.resetFields();
      onClose();
    },
    onError: (error: any) => {
      toast.error(`فشل في إنشاء معاملة الشركة: ${error.response?.data?.message || error.message}`);
    },
  });
