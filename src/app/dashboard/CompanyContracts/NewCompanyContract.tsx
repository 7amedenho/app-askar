...
interface NewCompanyContractProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function NewCompanyContract({
  isOpen,
  onClose,
}: NewCompanyContractProps) {
  const [form] = Form.useForm();
  const queryClient = useQueryClient();

  const mutation = useMutation({
    mutationFn: async (data: any) => {
      const response = await axios.post("/api/company-contracts", data);
